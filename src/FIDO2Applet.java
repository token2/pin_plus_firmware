...
public final class FIDO2Applet extends Applet implements ExtendedLength {
	...
	private boolean APPLET_INIT = false;
    /**
     ...
     */
    private static final short NUM_RESIDENT_KEY_SLOTS_PER_BATCH = 50;
    /**
    ...
    private static final short PIN_MIN_SIZE = 0x06;
	private static final short MIXED_PIN_MIN_SIZE = 0x0A;
	private static final short PIN_MAX_SIZE = 0x3F;
    /**
	...
	private static final short FIDO_MAX_COUNT = 0x12C;
     ...
     */
    private byte minPinLength = PIN_MIN_SIZE;
    private byte minMixedPinLength = MIXED_PIN_MIN_SIZE;
    /**
     ...
     */
    private Cipher sharedSecretWrapper;
    /**
     * decipher things sent by the platform
     */
    private Cipher sharedSecretUnwrapper;
    /**
     ...
     */
    private AESKey highSecurityWrappingKey;
    /**
     ...
     */
    private AESKey lowSecurityWrappingKey;
    /**
     ...
     */
    private Cipher symmetricWrapper;
    /**
     * decrypt data using authenticator master key
     */
    private Cipher symmetricUnwrapper;
    /**
     * Randomness.
     */
    ...
    private void makeCredential(APDU apdu, short lc, byte[] buffer) {
        ...
        if (transientStorage.hasRKOption()) {
            ...
            if (numResidentCredentials == 0) {
                ...
                for (short i = 0; i < FIDO_MAX_COUNT; i++) {
                    if (getResidenKey(i) == null) {
                        targetRKSlot = i;
                        break;
                    }
                    ...
                    if (checkCredential(
                        ...
                        if (userIdLen == getResidenKey(i).getUserIdLength()) {
                            // DECRYPT the encrypted user ID we stored for this RK, so we can compare
                            AESKey key = getAESKeyForExistingRK(i);
                            getResidenKey(i).unpackUserID(key, symmetricUnwrapper, scratchUserIdBuffer, scratchUserIdOffset);
            ...
            try {
                ...
                if (!foundMatchingRK) {
					...
					uniqueRP = getResidenKey(targetRKSlot).isUniqueRP();
                    ...
                    for (short i = targetRKSlot; i < (short)(numResidentCredentials - 1); i++) {
                    	setResidenKey(i, getResidenKey((short)(i + 1)));
                    }
				...
				final ResidentKeyData resKey = new ResidentKeyData(
                ...
                resKey.setEncryptedCredential(scratchCredBuffer, scratchCredOffset,
                        CREDENTIAL_ID_LEN, effectiveCPLevel, !lowSecWasUsed);

                resKey.setUser(key, symmetricWrapper,
                        scratchUserIdBuffer, scratchUserIdOffset, userIdLen,
                ...
                resKey.setRpId(key, symmetricWrapper,
                        scratchResidentRPIdBuffer, scratchResidentRPIDOffset, (byte) rpIdLen);

                setResidenKey(targetRKSlot, resKey);
        ...
        if (!selfAttestation) {
            ...
            if (largeBlobKeyRequested) {
                ...
                getResidenKey(targetRKSlot).emitLargeBlobKey(getAESKeyForExistingRK(targetRKSlot), symmetricWrapper,
    ...
    private short writeUVM(byte[] outBuf, short writeIdx, byte flags) {
        ...
        if (userVerified) {
            writeIdx = encodeIntTo(outBuf, writeIdx, (short) 0x0800); // user verification method passcode_external
        } else {
    ...
    private void getAssertion(final APDU apdu, final short lc, final byte[] buffer,
            ...
            for (short i = firstCred; i >= 0; i--) {
                if (checkCredential(apdu, i, scratchRPIDHashBuffer, scratchRPIDHashIdx,
                    ...
                    if (rkMatch == -1) {
                        ...
                        Util.arrayCopyNonAtomic(getResidenKey(i).getEncryptedCredentialID(), (short) 0,
                                credStorageBuffer, credStorageOffset, getResidenKey(i).getCredLen());
                        acceptedMatch = true;
        ...
        if (hmacSecretBytes > 0 || providingUVM || providingCredBlob) {
            ...
            if (providingCredBlob) {
                if (rkMatch != -1) {
                    credBlobBytes = getResidenKey(rkMatch).getCredBlobLen();
                }
        ...
        if (providingCredBlob) {
            ...
            if (credBlobBytes == 0) {
                ...
                short credBlobLen = getResidenKey(rkMatch).getCredBlobLen();
                ...
                getResidenKey(rkMatch).unpackCredBlob(getAESKeyForExistingRK(rkMatch), symmetricUnwrapper,
        ...
        if (rkMatch > -1) {
            final short uidLen = getResidenKey(rkMatch).getUserIdLength();
            ...
            getResidenKey(rkMatch).unpackUserID(getAESKeyForExistingRK(rkMatch), symmetricUnwrapper,
                    outputBuffer, outputIdx);
            outputIdx += uidLen; // only advance by the ACTUAL length of the UID, not the padded size

            if (returningUserName) {
                final short nameLength = getResidenKey(rkMatch).getUserNameLength();
                ...
                getResidenKey(rkMatch).unpackUserName(getAESKeyForExistingRK(rkMatch), symmetricUnwrapper,
                        outputBuffer, outputIdx);
        ...
        if (providingLBK) {
            ...
            getResidenKey(rkMatch).emitLargeBlobKey(
    ...
    private short scanRKsForExactCredential(byte[] buffer, short credIdx) {
        for (short i = 0; i < FIDO_MAX_COUNT; i++) {
            ...
            if (Util.arrayCompare(buffer, credIdx,
                    getResidenKey(i).getEncryptedCredentialID(), (short) 0, getResidenKey(i).getCredLen()) == 0) {
    ...
    private boolean checkCredential(APDU apdu, short rkNum,
        ...
        return checkCredential(apdu, getResidenKey(rkNum).getEncryptedCredentialID(), (short) 0, getResidenKey(rkNum).getCredLen(),
                rpIdBuf, rpIdHashIdx,
    ...
    private boolean checkCredential(APDU apdu, byte[] credentialBuffer, short credentialOffset, short credentialLen,
        ...
        if (rkNum >= 0) {
            ...
            byte credProtectLevel = getResidenKey(rkNum).getCredProtectLevel();
    ...
    public void process(APDU apdu) throws ISOException {
		...
		if (cla_ins == 0x0001) {
			...
			return;
		} else if (cla_ins == Util.makeShort((byte)0x80, FIDOConstants.CMD_CUSTOM_CONFIG)) {
			if (p1_p2 == 0x020A) {
				transientStorage.clearOutgoingContinuation();
				vendorCustomConfig(apdu);
				return;
			}
		}
    ...
    private void handleInit() {
		...
		residentKeys = new Object[6];
		for( short i=0; i<(short)residentKeys.length; i++)
		{
			residentKeys[i] = new ResidentKeyData[NUM_RESIDENT_KEY_SLOTS_PER_BATCH];
		}
    ...
    private boolean streamOutgoingContinuation(APDU apdu, byte[] apduBytes, boolean chaining) {
        ...
        if (chaining) {
            ...
            if (requestedChunkSize > (short)0 && requestedChunkSize < chunkSize) {
    ...
    private void authenticatorConfigSubcommand(APDU apdu, byte[] reqBuffer, short lc) {
        ...
        switch (reqBuffer[readIdx++]) {
            ...
            case FIDOConstants.AUTH_CONFIG_SET_MIN_PIN_LENGTH:
                setMinPin(apdu, reqBuffer, cmdParamsIdx, cmdParamsLen, lc, false);
                break;
     ...
     */
    private void setMinPin(APDU apdu, byte[] buffer, short readIdx, short cmdParamsLen, short lc, boolean isMixed) {
        if (cmdParamsLen == 0) {
		...
		byte tempMinPinLength;
		if (isMixed) {
			tempMinPinLength = minMixedPinLength;
		}
		else {
			tempMinPinLength = minPinLength;
		}
        byte newMinPINLength = tempMinPinLength;
    ...
    private void handleUserUpdate(APDU apdu, byte[] buffer, short readOffset, short lc) {
        ...
        for (short i = 0; i < FIDO_MAX_COUNT; i++) {
            ...
            if (Util.arrayCompare(getResidenKey(i).getEncryptedCredentialID(), (short) 0,
                ...
                boolean ok = false;
                try {
                	final ResidentKeyData resKey = getResidenKey(i);
                    resKey.setUser(getAESKeyForExistingRK(i), symmetricWrapper,
                            buffer, userIdIdx, userIdLen,
                            buffer, userNameIdx, userNameLen);
                    setResidenKey(i, resKey);
                    ok = true;
                } finally {
                    if (ok) {
                        JCSystem.commitTransaction();
    ...
    private void handleDeleteCred(APDU apdu, byte[] buffer, short readOffset, short lc) {
        ...
        for (short i = 0; i < FIDO_MAX_COUNT; i++) {
            ...
            if (Util.arrayCompare(getResidenKey(i).getEncryptedCredentialID(), (short) 0,
                    buffer, credIdIdx, getResidenKey(i).getCredLen()) == 0) {
                ...
                if (getResidenKey(i).isUniqueRP()) {
                    ...
                    for (short otherRKIdx = 0; otherRKIdx < FIDO_MAX_COUNT; otherRKIdx++) {
                        if (otherRKIdx == i) {
                            // we want ANOTHER RK, not this one...
                            continue;
                        }
                        if (getResidenKey(otherRKIdx) == null) {
                            // oops, hit the end
                            break;
                        }
                    ...
                    try {
                        if (rpHavingSameRP == -1) {
                        	...
                        	final ResidentKeyData resKey = getResidenKey(rpHavingSameRP);
                            resKey.setUniqueRP(true);
                            setResidenKey(rpHavingSameRP, resKey);
                        }
                        for (short j = i; j < (short)(numResidentCredentials - 1); j++) {
                        	setResidenKey(j, getResidenKey((short)(j + 1)));
                        }
                        setResidenKey(--numResidentCredentials, null);
                        ok = true;
                    } finally {
                        if (ok) {
                            JCSystem.commitTransaction();
    ...
    private void handleEnumerateCreds(APDU apdu, byte[] buffer, short bufferIdx, short startCredIdx, short lc) {
        ...
        if (startCredIdx > FIDO_MAX_COUNT) { // intentional > instead of >=
            sendErrorByte(apdu, FIDOConstants.CTAP2_ERR_NO_CREDENTIALS);
        }
        ...
        if (startCredIdx == 0) {
            ...
            extractRKMixed(getResidenKey(iterIdx).getEncryptedCredentialID(), (short) 0,
                    rpIdHashBuf, rpIdHashIdx, iterIdx);
        }

        short rkIndex;
        for (rkIndex = startCredIdx; rkIndex < FIDO_MAX_COUNT; rkIndex++) {
            if (getResidenKey(rkIndex) == null) {
                break;
            }
            ...
            if (checkCredential(
                ...
                if (startCredIdx == 0) {
                    ...
                    for (short otherCredIdx = (short) (rkIndex + 1); otherCredIdx < FIDO_MAX_COUNT; otherCredIdx++) {
                        if (getResidenKey(otherCredIdx) == null) {
                            break;
                        }
                ...
                final short userNameLength = getResidenKey(rkIndex).getUserNameLength();
                ...
                final short userIdLength = getResidenKey(rkIndex).getUserIdLength();
                ...
                getResidenKey(rkIndex).unpackUserID(key, symmetricUnwrapper,
                        outBuf, writeOffset);
                ...
                if (userNameLength > 0) {
                    ...
                    getResidenKey(rkIndex).unpackUserName(key, symmetricUnwrapper,
                            outBuf, writeOffset);
                    writeOffset += userNameLength;
                }

                outBuf[writeOffset++] = 0x07; // map key: credentialId
                writeOffset = packCredentialId(getResidenKey(rkIndex).getEncryptedCredentialID(), (short) 0,
                        outBuf, writeOffset);
                ...
                getResidenKey(rkIndex).unpackPublicKey(
                        pkBuf, pkBufIdx);
                ...
                outBuf[writeOffset++] = getResidenKey(rkIndex).getCredProtectLevel();
    ...
    private void updateRKStatekeeping(APDU apdu) {
        ...
        for (short rkIndex1 = 0; rkIndex1 < FIDO_MAX_COUNT; rkIndex1++) {
            if (getResidenKey(rkIndex1) == null) {
                break;
            }
            if (!getResidenKey(rkIndex1).isUniqueRP()) {
                // definitely non-unique (or inactive) RP
                continue;
            }
            numUniqueRPsFound++;
            extractRKMixed(getResidenKey(rkIndex1).getEncryptedCredentialID(), (short) 0,
                    rp2Buffer, rp2Offset, rkIndex1);
            Util.arrayCopyNonAtomic(rp2Buffer, rp2Offset,
                    rp1Buffer, rp1Offset, RP_HASH_LEN);
            for (short rkIndex2 = (short)(rkIndex1 + 1); rkIndex2 < FIDO_MAX_COUNT; rkIndex2++) {
                if (getResidenKey(rkIndex2) == null) {
                    break;
                }
                if (!getResidenKey(rkIndex2).isUniqueRP()) {
                    // definitely non-unique RP
                    continue;
                }

                // Two different RPs; compare!
                extractRKMixed(getResidenKey(rkIndex2).getEncryptedCredentialID(), (short) 0,
                ...
                if (Util.arrayCompare(rp1Buffer, rp1Offset,
                    ...
                    getResidenKey(rkIndex2).setUniqueRP(false);
                }
            }
        }
    ...
    private void handleEnumerateRPs(APDU apdu, short startOffset) {
        ...
        short rkIndex;
        for (rkIndex = startOffset; rkIndex < FIDO_MAX_COUNT; rkIndex++) {
            // if a credential is not for a *unique* RP, ignore it - we're enumerating RPs here!
            if (getResidenKey(rkIndex) == null) {
                break;
            }
            if (getResidenKey(rkIndex).isUniqueRP()) {
        ...
        byte rpIdLength = getResidenKey(rkIndex).getRpIdLength();
        writeOffset = encodeIntLenTo(outBuf, writeOffset, rpIdLength, false);

        // Decrypt the full RP ID, ...
        getResidenKey(rkIndex).unpackRpId(getAESKeyForExistingRK(rkIndex), symmetricUnwrapper,
                outBuf, writeOffset);
        ...
        extractRKMixed(getResidenKey(rkIndex).getEncryptedCredentialID(), (short) 0,
    ...
    private AESKey getAESKeyForExistingRK(short rkIndex) {
        if (getResidenKey(rkIndex).getHighSecEncrypted()) {
    ...
    private short encodeIntTo(byte[] outBuf, short writeOffset, short v) {
        if (v < 24) {
            ...
            outBuf[writeOffset++] = 0x19; // Integer stored in one byte
            Util.setShort(outBuf, writeOffset, v);
            writeOffset += 2;
        }
        return writeOffset;
    }
    ...
    private void authenticatorReset(APDU apdu) {
        ...
        try {
            ...
            minPinLength = PIN_MIN_SIZE;
            minMixedPinLength = MIXED_PIN_MIN_SIZE;
    ...
    private void sendAuthInfo(APDU apdu) {
		...
		offset = encodeIntTo(buffer, offset, (short) bufferMem.length);
        ...
        final short approximateKeyCount = getApproximateRemainingKeyCount();
        ...
        if (!longResponse) {
            ...
            // buffer. Write aside.
            short remainingAmountToWrite = (short)CannedCBOR.ES256_ALG_TYPE.length;
            if (approximateKeyCount > (short)255) { // encoded length of approxKeyCount
	            remainingAmountToWrite += 3;
            } else if (approximateKeyCount > 23) {
	            remainingAmountToWrite += 2;
            } else {
	            remainingAmountToWrite += 1;
            }
            remainingAmountToWrite += (minPinLength > 23 ? 2 : 1); // encoded length of minPinLength
			remainingAmountToWrite += 23; // fixed overhead;
        ...
        offset = encodeIntTo(buffer, offset, CREDENTIAL_ID_LEN); // 2 bytes = 3
        
        offset = Util.arrayCopyNonAtomic(CannedCBOR.AUTH_INFO_FOURTH, (short) 0,
                buffer, offset, (short) CannedCBOR.AUTH_INFO_FOURTH.length);
        ...
        offset = encodeIntTo(buffer, offset, (short) getCurrentLargeBlobStore().length); // 2 bytes = 8
        ...
        offset = encodeIntTo(buffer, offset, ub(minPinLength)); // encoded separately

        buffer[offset++] = 0x0E; // map key: firmwareVersion: 1 byte = 12
        offset = encodeIntTo(buffer, offset, ub(FIRMWARE_VERSION)); // 1 byte = 13

        buffer[offset++] = 0x0F; // map key: maxCredBlobLength: 1 byte = 14
        offset = encodeIntTo(buffer, offset, ub(MAX_CRED_BLOB_LEN)); // 2 bytes = 16

        buffer[offset++] = 0x10; // map key: maxRPIDsForSetMinPinLength: 1 byte = 17
        offset = encodeIntTo(buffer, offset, ub(MAX_RP_IDS_MIN_PIN_LENGTH)); // 1 byte = 18

        buffer[offset++] = 0x12; // map key: uvModality: 1 byte = 19
        offset = encodeIntTo(buffer, offset, (short) 0x0200); // uvModality "none": 2 bytes = 22
     ...
     */
    private short getApproximateRemainingKeyCount() {
        return (short)(FIDO_MAX_COUNT - numResidentCredentials);
    }
    ...
    private void rawSetPIN(APDU apdu, byte[] pinBuf, short offset, short pinLength) {
        if (pinLength < minPinLength || pinLength > PIN_MAX_SIZE) {
            sendErrorByte(apdu, FIDOConstants.CTAP2_ERR_PIN_POLICY_VIOLATION);
        }
		...
		// only nunber
		if ( CustomFunction.IsCorrectFormat(pinBuf, offset, pinLength, CustomFunction.DIGIT_CHAR, (short)1) )
		{
			if ( pinLength > PIN_MAX_SIZE || pinLength < minPinLength )
			{
				sendErrorByte(apdu, FIDOConstants.CTAP2_ERR_PIN_POLICY_VIOLATION);
			}
			if ( !CustomFunction.IsCorrectDelta(pinBuf, offset, pinLength) 
					|| !CustomFunction.IsNonMirrored(pinBuf, offset, pinLength) 
					|| !CustomFunction.IsNonContinuousChar(pinBuf, offset, pinLength, (short)3) )
			{
				sendErrorByte(apdu, FIDOConstants.CTAP2_ERR_PIN_POLICY_VIOLATION);
			}
		}
		// mixed pin
		else if ( CustomFunction.IsCorrectFormat(pinBuf, offset, pinLength, (byte)0x0F, (short)2) )
		{
			if ( pinLength > PIN_MAX_SIZE || pinLength < minMixedPinLength )
			{
				sendErrorByte(apdu, FIDOConstants.CTAP2_ERR_PIN_POLICY_VIOLATION);
			}
		}
    ...
    private void initAuthenticatorKey(boolean authenticatorKeyInRam) {
        ...
        ECPublicKey tECPublicKey = (ECPublicKey) KeyBuilder.buildKey(KeyBuilder.TYPE_EC_FP_PUBLIC, KeyBuilder.LENGTH_EC_FP_256, false);
        ECPrivateKey tECPrivateKey = getECPrivKey();
        P256Constants.setCurve(tECPublicKey);
        P256Constants.setCurve(tECPrivateKey);
        authenticatorKeyAgreementKey = new KeyPair(tECPublicKey, tECPrivateKey);
    }
     ...
     */
     private void initCredKey(boolean ecPairInRam) {
        ...
        P256Constants.setCurve(tECPublicKey);
        P256Constants.setCurve(tECPrivateKey);
        ecKeyPair = new KeyPair(tECPublicKey, tECPrivateKey);
    }
    ...
    private void initTransientStorage(APDU apdu) {
        ...
        sharedSecretVerifyKey = JCSystem.makeTransientByteArray((short) 32, JCSystem.CLEAR_ON_DESELECT);
    ...
    private void vendorCustomConfig(APDU apdu) {
    	final short amtRead = apdu.setIncomingAndReceive();
        short lc = apdu.getIncomingLength();
        if (lc == 0) {
            apdu.getBuffer()[0] = minMixedPinLength;
            sendNoCopy(apdu, (short) 1);
            return;
        }
        final byte[] reqBuffer = fullyReadReq(apdu, lc, amtRead, false);
        short readIdx = (short) 1;
        ...
        switch (reqBuffer[readIdx++]) {
            case FIDOConstants.AUTH_CONFIG_SET_MIN_PIN_LENGTH:
                setMinPin(apdu, reqBuffer, cmdParamsIdx, cmdParamsLen, lc, true);
                break;
            default:
                // Spec says this is INVALID_PARAMETER, not CTAP2 INVALID_SUBCOMMAND
                sendErrorByte(apdu, FIDOConstants.CTAP1_ERR_INVALID_PARAMETER);
    ...
    private ResidentKeyData getResidenKey(short rkIndex) {
    	short s2Arr = (short)(rkIndex/NUM_RESIDENT_KEY_SLOTS_PER_BATCH);
		short s2Idx = (short)(rkIndex%NUM_RESIDENT_KEY_SLOTS_PER_BATCH);
		final ResidentKeyData[] tempKeys = (ResidentKeyData[])residentKeys[s2Arr];
		
		return tempKeys[s2Idx];
    }
    
    private void setResidenKey(short rkIndex, ResidentKeyData tempKey) {
    	short s2Arr = (short)(rkIndex/NUM_RESIDENT_KEY_SLOTS_PER_BATCH);
		short s2Idx = (short)(rkIndex%NUM_RESIDENT_KEY_SLOTS_PER_BATCH);
		final ResidentKeyData[] tempKeys = (ResidentKeyData[])residentKeys[s2Arr];
		
		tempKeys[s2Idx] = tempKey;
    }
    
    private void clearResidenKey() {
	    for (short i = 0; i < (short)residentKeys.length; i++ ) {
		    final ResidentKeyData[] tempKeys = (ResidentKeyData[])residentKeys[i];
		    for (short j = 0; j < NUM_RESIDENT_KEY_SLOTS_PER_BATCH; j++) {
			    tempKeys[j] = null;
		    }
	    }
    }
